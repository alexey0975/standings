...
document.addEventListener('DOMContentLoaded', () => {
    ...
    Array.from(radioBtns).map((btn) => {
        btn.addEventListener('change', () => __awaiter(void 0, void 0, void 0, function* () {
            const setLoader = setTimeout(() => {
                tableContainer && tableContainer.append(loader);
            }, 100);
            try {
                const data = yield getTable(btn.name, btn.value);
                if (tableContainer) {
    ...
    <table class="table">
      ...
      ${data.data.result.map((car) => {
          ...
          <tr class="table__row table__row_data">
            ...
            ${data.choice === 'all'
                ? car.results.map((value, index) => (`<td class="table__cell" data-heading="${index + 1}-й заезд">${value}</td>`)).join('\n') +
    ...
    function getTable(name, value) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const response = yield fetch(`/src/api.php?${name}=${value}`, {
                    method: "GET",
                });
